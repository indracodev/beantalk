"use strict";var BeanTalk=(()=>{var S=Object.defineProperty;var $=Object.getOwnPropertyDescriptor;var G=Object.getOwnPropertyNames;var V=Object.prototype.hasOwnProperty;var K=(n,e)=>{for(var t in e)S(n,t,{get:e[t],enumerable:!0})},Y=(n,e,t,i)=>{if(e&&typeof e=="object"||typeof e=="function")for(let s of G(e))!V.call(n,s)&&s!==t&&S(n,s,{get:()=>e[s],enumerable:!(i=$(e,s))||i.enumerable});return n};var Q=n=>Y(S({},"__esModule",{value:!0}),n);var he={};K(he,{BeanTalk:()=>g,ChatWidget:()=>d,UniversalChatMe:()=>ie,close:()=>ae,default:()=>ce,getInstance:()=>de,init:()=>ne,on:()=>re,open:()=>se,sendMessage:()=>le,toggle:()=>oe});var x=class{constructor(){this.events={}}on(e,t){return this.events[e]||(this.events[e]=[]),this.events[e].push(t),this}off(e,t){return this.events[e]?(this.events[e]=this.events[e].filter(i=>i!==t),this):this}emit(e,t){this.events[e]&&this.events[e].forEach(i=>{try{i(t)}catch(s){console.error(`[BeanTalk] Error in event handler for "${e}":`,s)}})}};var b=class{constructor(e,t=""){this.projectKey=e,this.baseUrl=t.replace(/\/+$/,"")}async request(e,t={}){let i=`${this.baseUrl}${e}`,s={Accept:"application/json","Content-Type":"application/json","X-Project-Key":this.projectKey,...t.headers||{}},o=new AbortController,a=setTimeout(()=>o.abort(),12e3);try{let l=await fetch(i,{...t,headers:s,signal:o.signal});return clearTimeout(a),await l.json()}catch(l){return clearTimeout(a),{success:!1,error:{code:l.name==="AbortError"?"TIMEOUT":"NETWORK_ERROR",message:l.message||"Gagal terhubung ke server chat."}}}}async initSession(e,t,i){return this.request("/api/v1/client/session/init",{method:"POST",body:JSON.stringify({visitor_uuid:e,name:i,project_key:this.projectKey,page_url:window.location.href,page_title:document.title,client_url:window.location.href,metadata:{referrer:document.referrer,userAgent:navigator.userAgent,title:document.title,...t}})})}async updateProfile(e,t){return this.request("/api/v1/client/session/profile",{method:"POST",body:JSON.stringify({visitor_uuid:e,name:t})})}async pollMessages(e,t=0){return this.request(`/api/v1/client/conversations/${e}/messages?after_id=${t}`,{method:"GET"})}async sendMessage(e,t){let i=e&&e>0?e:0;return this.request(`/api/v1/client/conversations/${i}/messages`,{method:"POST",body:JSON.stringify({visitor_uuid:t.visitor_uuid,client_message_id:t.client_message_id,content:t.message,message:t.message,sender_name:t.sender_name,page_url:t.page_url,page_title:t.page_title})})}};var H="beantalk_visitor_uuid",J="beantalk_last_conv_id",O={};function j(n){try{return window.localStorage.getItem(n)}catch{return O[n]||null}}function F(n,e){try{window.localStorage.setItem(n,e)}catch{O[n]=e}}function X(){return typeof crypto<"u"&&typeof crypto.randomUUID=="function"?crypto.randomUUID():"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,n=>{let e=Math.random()*16|0;return(n==="x"?e:e&3|8).toString(16)})}function v(){return"msg_"+Date.now().toString(36)+"_"+Math.random().toString(36).substring(2,9)}function y(){let n=j(H);return n||(n=X(),F(H,n)),n}function M(n){F(J,n.toString())}var D="beantalk_customer_name";function u(){return j(D)}function z(n){F(D,n)}var f=class{constructor(e,t,i={}){this.conversationId=null;this.lastMessageId=0;this.isRunning=!1;this.isPolling=!1;this.pollQueued=!1;this.timer=null;this.isWindowVisible=!0;this.isOnline=!0;this.isWidgetOpen=!1;this.burstRemaining=0;this.burstIntervalMs=1200;this.api=e,this.emitter=t,this.activeIntervalMs=i.activeIntervalMs||2500,this.idleIntervalMs=i.idleIntervalMs||15e3,this.setupListeners()}setupListeners(){typeof document<"u"&&document.addEventListener("visibilitychange",()=>{let e=document.visibilityState==="visible";this.isWindowVisible=e,e&&this.isRunning&&this.pollNow()}),typeof window<"u"&&(window.addEventListener("online",()=>{this.isOnline=!0,this.isRunning&&this.pollNow()}),window.addEventListener("offline",()=>{this.isOnline=!1,this.clearTimer()}))}setConversation(e,t=0){this.conversationId=e,t>this.lastMessageId&&(this.lastMessageId=t)}setWidgetOpen(e){this.isWidgetOpen=e,this.isRunning&&this.reschedule()}start(e,t=0){this.conversationId=e,this.lastMessageId=Math.max(this.lastMessageId,t),this.isRunning=!0,this.reschedule()}stop(){this.isRunning=!1,this.clearTimer()}clearTimer(){this.timer&&(clearTimeout(this.timer),this.timer=null)}getInterval(){return this.burstRemaining>0?this.burstIntervalMs:this.isWindowVisible&&this.isWidgetOpen?this.activeIntervalMs:this.idleIntervalMs}reschedule(){this.clearTimer(),!(!this.isRunning||!this.isOnline)&&(this.timer=setTimeout(()=>{this.executePoll()},this.getInterval()))}async pollNow(){if(this.isPolling){this.pollQueued=!0;return}this.clearTimer(),this.burstRemaining=3,await this.executePoll()}async executePoll(){if(!this.isRunning||!this.conversationId||!this.isOnline||this.isPolling){this.reschedule();return}this.isPolling=!0;try{let e=await this.api.pollMessages(this.conversationId,this.lastMessageId);if(e.success&&e.data){let{messages:t,last_id:i}=e.data;if(Array.isArray(t)&&t.length>0){let s=t.filter(o=>o.id>this.lastMessageId);s.length>0&&s.forEach(o=>{this.emitter.emit("message:received",o)}),i&&i>this.lastMessageId&&(this.lastMessageId=i)}}}catch(e){this.emitter.emit("poll:error",e)}finally{this.isPolling=!1,this.burstRemaining>0&&this.burstRemaining--,this.pollQueued?(this.pollQueued=!1,this.burstRemaining=2,this.clearTimer(),this.timer=setTimeout(()=>this.executePoll(),100)):this.reschedule()}}};var r={chat:`<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  `}function Z(n,e){let t=parseInt(n.replace("#",""),16),i=(t>>16)+e,s=(t>>8&255)+e,o=(t&255)+e;return i=Math.min(255,Math.max(0,i)),s=Math.min(255,Math.max(0,s)),o=Math.min(255,Math.max(0,o)),"#"+(s|o<<8|i<<16).toString(16).padStart(6,"0")}var w=class{constructor(e,t){this.isOpen=!1;this.currentStage="welcome";this.unreadCount=0;this.messages=[];this.sessionData=null;this.customerName="";this.customerCode="";this.audioCtx=null;this.options=e,this.emitter=t;let i=document.getElementById("beantalk-chat-root")||document.getElementById("universal-chat-root");i||(i=document.createElement("div"),i.id="beantalk-chat-root",i.style.position="relative",i.style.zIndex="2147483647",i.style.display="block",document.body.appendChild(i)),this.shadowRoot=i.attachShadow({mode:"open"}),this.styleEl=document.createElement("style"),this.styleEl.textContent=A(e.accentColor||"#1E1E1E"),this.shadowRoot.appendChild(this.styleEl),this.renderSkeleton(),this.bindEvents();let s=u();s&&this.applyCustomerName(s,!1)}updateTheming(e){this.styleEl&&(this.styleEl.textContent=A(e))}setSessionData(e){this.sessionData=e;let t=e.widget||e.widget_settings||{};t.primary_color&&this.updateTheming(t.primary_color);let i=t.greeting_title||t.header_title||e.project?.name||"Chat Support",s=t.greeting_subtitle||t.greeting_text||"Hallo! Ada yang bisa kami bantu? Tanyakan apapun di sini.",o=this.shadowRoot.querySelector(".welcome-title");o&&(o.textContent=i);let a=this.shadowRoot.querySelector(".welcome-subtitle");a&&(a.textContent=s);let l=t.support_title||this.options.supportTitle||this.options.brandName||this.options.storeName||e.project?.name||"Customer Support",m=this.shadowRoot.querySelector("#cardChatName");m&&(m.textContent=l),this.identitySupportTag&&(this.identitySupportTag.textContent=l);let E=this.shadowRoot.querySelector(".chat-header-title");E&&(E.textContent=l);let B=this.shadowRoot.querySelector(".welcome-brand-badge");B&&(B.textContent=e.project?.name||"Live Support");let k=e.visitor||{},T=k.customer_code||k.customer_code_formatted;T&&(this.customerCode=T,this.identityCustCode&&(this.identityCustCode.textContent=T));let U=k.name,W=u(),R=U||W;R?this.applyCustomerName(R,!1):this.chatInlineIdentityBanner&&(this.chatInlineIdentityBanner.style.display="flex"),e.conversation?.messages&&e.conversation.messages.length>0&&this.setMessages(e.conversation.messages);let C=this.shadowRoot.querySelector("#socialChannelsCard"),L=this.shadowRoot.querySelector("#socialChannelsTitle"),I=this.shadowRoot.querySelector("#socialChannelsRow"),_=t.social_channels||[],q=t.find_us_title||"Reach Us Anywhere Else";if(L&&(L.textContent=q),C&&I){let N=Array.isArray(_)?_.filter(p=>p.enabled&&p.url):[];N.length>0?(I.innerHTML="",N.forEach(p=>{let h=document.createElement("a");h.className=`social-channel-btn social-btn-${p.id}`,h.href=p.url,h.target="_blank",h.rel="noopener noreferrer",h.title=`Hubungi via ${p.name}`;let P=r[p.icon||p.id]||r.chat;h.innerHTML=P,I.appendChild(h)}),C.style.display="block"):C.style.display="none"}}applyCustomerName(e,t=!0){let i=e.trim();i&&(this.customerName=i,t&&(z(i),this.emitter.emit("customer:rename",i)),this.identityNameInput&&(this.identityNameInput.value=i),this.chatInlineIdentityBanner&&(this.chatInlineIdentityBanner.style.display="none"),this.composerInput&&(this.composerInput.placeholder=`Tulis pesan sebagai ${i}...`))}renderSkeleton(){let e=this.options.brandName||this.options.storeName||"Customer Support",t=this.options.supportTitle||e||"Customer Support",i=this.options.greetingTitle||"Hallo!",s=this.options.greetingSubtitle||"Apakah ada yang bisa kami bantu? Tanyakan informasi apapun di sini.",a=`
