"use strict";var BeanTalk=(()=>{var N=Object.defineProperty;var Z=Object.getOwnPropertyDescriptor;var X=Object.getOwnPropertyNames;var ee=Object.prototype.hasOwnProperty;var te=(n,e)=>{for(var t in e)N(n,t,{get:e[t],enumerable:!0})},ie=(n,e,t,i)=>{if(e&&typeof e=="object"||typeof e=="function")for(let s of X(e))!ee.call(n,s)&&s!==t&&N(n,s,{get:()=>e[s],enumerable:!(i=Z(e,s))||i.enumerable});return n};var ne=n=>ie(N({},"__esModule",{value:!0}),n);var ye={};te(ye,{BeanTalk:()=>k,ChatWidget:()=>u,UniversalChatMe:()=>de,close:()=>ue,default:()=>ve,getInstance:()=>be,init:()=>pe,on:()=>me,open:()=>he,sendMessage:()=>xe,toggle:()=>ge});var C=class{constructor(){this.events={}}on(e,t){return this.events[e]||(this.events[e]=[]),this.events[e].push(t),this}off(e,t){return this.events[e]?(this.events[e]=this.events[e].filter(i=>i!==t),this):this}emit(e,t){this.events[e]&&this.events[e].forEach(i=>{try{i(t)}catch(s){console.error(`[BeanTalk] Error in event handler for "${e}":`,s)}})}};var I=class{constructor(e,t=""){this.projectKey=e,this.baseUrl=t.replace(/\/+$/,"")}async request(e,t={}){let i=`${this.baseUrl}${e}`,s={Accept:"application/json","Content-Type":"application/json","X-Project-Key":this.projectKey,...t.headers||{}},a=new AbortController,l=setTimeout(()=>a.abort(),12e3);try{let o=await fetch(i,{...t,headers:s,signal:a.signal});return clearTimeout(l),await o.json()}catch(o){return clearTimeout(l),{success:!1,error:{code:o.name==="AbortError"?"TIMEOUT":"NETWORK_ERROR",message:o.message||"Gagal terhubung ke server chat."}}}}async initSession(e,t,i){return this.request("/api/v1/client/session/init",{method:"POST",body:JSON.stringify({visitor_uuid:e,name:i,project_key:this.projectKey,page_url:window.location.href,page_title:document.title,client_url:window.location.href,metadata:{referrer:document.referrer,userAgent:navigator.userAgent,title:document.title,...t}})})}async updateProfile(e,t){return this.request("/api/v1/client/session/profile",{method:"POST",body:JSON.stringify({visitor_uuid:e,name:t})})}async pollMessages(e,t=0){return this.request(`/api/v1/client/conversations/${e}/messages?after_id=${t}`,{method:"GET"})}async sendMessage(e,t){let i=e&&e>0?e:0;return this.request(`/api/v1/client/conversations/${i}/messages`,{method:"POST",body:JSON.stringify({visitor_uuid:t.visitor_uuid,client_message_id:t.client_message_id,content:t.message,message:t.message,sender_name:t.sender_name,page_url:t.page_url,page_title:t.page_title})})}};var W="beantalk_visitor_uuid",se="beantalk_last_conv_id",q={};function V(n){try{return window.localStorage.getItem(n)}catch{return q[n]||null}}function H(n,e){try{window.localStorage.setItem(n,e)}catch{q[n]=e}}function ae(){return typeof crypto<"u"&&typeof crypto.randomUUID=="function"?crypto.randomUUID():"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,n=>{let e=Math.random()*16|0;return(n==="x"?e:e&3|8).toString(16)})}function T(){return"msg_"+Date.now().toString(36)+"_"+Math.random().toString(36).substring(2,9)}function A(){let n=V(W);return n||(n=ae(),H(W,n)),n}function O(n){H(se,n.toString())}var K="beantalk_customer_name";function w(){return V(K)}function G(n){H(K,n)}var M=class{constructor(e,t,i={}){this.conversationId=null;this.lastMessageId=0;this.isRunning=!1;this.isPolling=!1;this.pollQueued=!1;this.timer=null;this.isWindowVisible=!0;this.isOnline=!0;this.isWidgetOpen=!1;this.burstRemaining=0;this.burstIntervalMs=1200;this.api=e,this.emitter=t,this.activeIntervalMs=i.activeIntervalMs||2500,this.idleIntervalMs=i.idleIntervalMs||15e3,this.setupListeners()}setupListeners(){typeof document<"u"&&document.addEventListener("visibilitychange",()=>{let e=document.visibilityState==="visible";this.isWindowVisible=e,e&&this.isRunning&&this.pollNow()}),typeof window<"u"&&(window.addEventListener("online",()=>{this.isOnline=!0,this.isRunning&&this.pollNow()}),window.addEventListener("offline",()=>{this.isOnline=!1,this.clearTimer()}))}setConversation(e,t=0){this.conversationId=e,t>this.lastMessageId&&(this.lastMessageId=t)}setWidgetOpen(e){this.isWidgetOpen=e,this.isRunning&&this.reschedule()}start(e,t=0){this.conversationId=e,this.lastMessageId=Math.max(this.lastMessageId,t),this.isRunning=!0,this.reschedule()}stop(){this.isRunning=!1,this.clearTimer()}clearTimer(){this.timer&&(clearTimeout(this.timer),this.timer=null)}getInterval(){return this.burstRemaining>0?this.burstIntervalMs:this.isWindowVisible&&this.isWidgetOpen?this.activeIntervalMs:this.idleIntervalMs}reschedule(){this.clearTimer(),!(!this.isRunning||!this.isOnline)&&(this.timer=setTimeout(()=>{this.executePoll()},this.getInterval()))}async pollNow(){if(this.isPolling){this.pollQueued=!0;return}this.clearTimer(),this.burstRemaining=3,await this.executePoll()}async executePoll(){if(!this.isRunning||!this.conversationId||!this.isOnline||this.isPolling){this.reschedule();return}this.isPolling=!0;try{let e=await this.api.pollMessages(this.conversationId,this.lastMessageId);if(e.success&&e.data){let{messages:t,last_id:i}=e.data;if(Array.isArray(t)&&t.length>0){let s=t.filter(a=>a.id>this.lastMessageId);s.length>0&&s.forEach(a=>{this.emitter.emit("message:received",a)}),i&&i>this.lastMessageId&&(this.lastMessageId=i)}}}catch(e){this.emitter.emit("poll:error",e)}finally{this.isPolling=!1,this.burstRemaining>0&&this.burstRemaining--,this.pollQueued?(this.pollQueued=!1,this.burstRemaining=2,this.clearTimer(),this.timer=setTimeout(()=>this.executePoll(),100)):this.reschedule()}}};var r={chat:`<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path>
    <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path>
  </svg>`};function P(n="#1E1E1E"){return`
    :host {
      all: initial;
  ...
  `}function oe(n,e){let t=parseInt(n.replace("#",""),16),i=(t>>16)+e,s=(t>>8&255)+e,a=(t&255)+e;return i=Math.min(255,Math.max(0,i)),s=Math.min(255,Math.max(0,s)),a=Math.min(255,Math.max(0,a)),"#"+(s|a<<8|i<<16).toString(16).padStart(6,"0")}var B=class{constructor(e,t){this.isOpen=!1;this.currentStage="welcome";this.unreadCount=0;this.messages=[];this.sessionData=null;this.customerName="";this.customerCode="";this.audioCtx=null;this.options=e,this.emitter=t;let i=document.getElementById("beantalk-chat-root")||document.getElementById("universal-chat-root");i||(i=document.createElement("div"),i.id="beantalk-chat-root",i.style.position="relative",i.style.zIndex="2147483647",i.style.display="block",document.body.appendChild(i)),this.shadowRoot=i.attachShadow({mode:"open"}),this.styleEl=document.createElement("style"),this.styleEl.textContent=P(e.accentColor||"#1E1E1E"),this.shadowRoot.appendChild(this.styleEl),this.renderSkeleton(),this.bindEvents(),this.initViewportHandler();let s=w();s&&this.applyCustomerName(s,!1)}updateTheming(e){this.styleEl&&(this.styleEl.textContent=P(e))}setSessionData(e){this.sessionData=e;let t=e.widget||e.widget_settings||{};t.primary_color&&this.updateTheming(t.primary_color);let i=t.greeting_title||t.header_title||e.project?.name||"Chat Support",s=t.greeting_subtitle||t.greeting_text||"Hallo! Ada yang bisa kami bantu? Tanyakan apapun di sini.",a=this.shadowRoot.querySelector(".welcome-title");a&&(a.textContent=i);let l=this.shadowRoot.querySelector(".welcome-subtitle");l&&(l.textContent=s);let o=t.support_title||this.options.supportTitle||this.options.brandName||this.options.storeName||e.project?.name||"Customer Support",p=this.shadowRoot.querySelector("#cardChatName");p&&(p.textContent=o),this.identitySupportTag&&(this.identitySupportTag.textContent=o);let c=this.shadowRoot.querySelector(".chat-header-title");c&&(c.textContent=o);let b=this.shadowRoot.querySelector(".welcome-brand-badge");b&&(b.textContent=e.project?.name||"Live Support");let g=e.visitor||{},m=g.customer_code||g.customer_code_formatted;m&&(this.customerCode=m,this.identityCustCode&&(this.identityCustCode.textContent=m));let L=g.name,R=w(),f=L||R;f?this.applyCustomerName(f,!1):this.chatInlineIdentityBanner&&(this.chatInlineIdentityBanner.style.display="flex"),e.conversation?.messages&&e.conversation.messages.length>0&&this.setMessages(e.conversation.messages);let E=this.shadowRoot.querySelector("#socialChannelsCard"),$=this.shadowRoot.querySelector("#socialChannelsTitle"),F=this.shadowRoot.querySelector("#socialChannelsRow"),j=t.social_channels||[],Y=t.find_us_title||"Reach Us Anywhere Else";if($&&($.textContent=Y),E&&F){let z=Array.isArray(j)?j.filter(y=>y.enabled&&y.url):[];if(z.length>0){F.innerHTML="";let y={};z.forEach(h=>{let v=(h.platform||h.icon||h.id||"whatsapp").toLowerCase();y[v]||(y[v]=[]),y[v].push(h)}),Object.keys(y).forEach(h=>{let v=y[h],_=v[0],Q=_.icon||h,U=r[Q]||r[h]||r.chat;if(v.length===1){let d=document.createElement("a");d.className=`social-channel-btn social-btn-${h}`,d.href=_.url,d.target="_blank",d.rel="noopener noreferrer",d.title=`Hubungi via ${_.name||D(h)}`,d.innerHTML=U,F.appendChild(d)}else{let d=document.createElement("button");d.type="button",d.className=`social-channel-btn social-btn-${h} has-multi-badge`,d.title=`${D(h)} (${v.length} pilihan kontak)`,d.innerHTML=`
              ${U}
              <span class="social-channel-badge">${v.length}</span>
            `,d.addEventListener("click",J=>{J.stopPropagation(),this.openSocialPicker(h,v)}),F.appendChild(d)}}),E.style.display="block"}else E.style.display="none"}}applyCustomerName(e,t=!0){let i=e.trim();i&&(this.customerName=i,t&&(G(i),this.emitter.emit("customer:rename",i)),this.identityNameInput&&(this.identityNameInput.value=i),this.chatInlineIdentityBanner&&(this.chatInlineIdentityBanner.style.display="none"),this.composerInput&&(this.composerInput.placeholder=`Tulis pesan sebagai ${i}...`))}renderSkeleton(){let e=this.options.brandName||this.options.storeName||"Customer Support",t=this.options.supportTitle||e||"Customer Support",i=this.options.greetingTitle||"Hallo!",s=this.options.greetingSubtitle||"Apakah ada yang bisa kami bantu? Tanyakan informasi apapun di sini.",l=`
    ...
    `,o=document.createElement("div");o.innerHTML=l,this.shadowRoot.appendChild(o.firstElementChild),this.wrapperEl=this.shadowRoot.querySelector(".chat-wrapper"),this.launcherBtn=this.shadowRoot.querySelector(".chat-launcher-btn"),this.unreadBadge=this.shadowRoot.querySelector(".launcher-unread-badge"),this.stageWelcome=this.shadowRoot.querySelector(".stage-welcome"),this.stageIdentity=this.shadowRoot.querySelector(".stage-identity"),this.stageChat=this.shadowRoot.querySelector(".stage-chat"),this.stageSocialPicker=this.shadowRoot.querySelector("#stageSocialPicker"),this.messagesArea=this.shadowRoot.querySelector(".chat-messages-area"),this.composerInput=this.shadowRoot.querySelector(".composer-textarea"),this.composerSendBtn=this.shadowRoot.querySelector(".composer-send-btn"),this.cardSnippetText=this.shadowRoot.querySelector("#card-snippet"),this.identityCustCode=this.shadowRoot.querySelector("#identityCustCode"),this.identityNameInput=this.shadowRoot.querySelector("#identityNameInput"),this.identityContinueBtn=this.shadowRoot.querySelector("#identityContinueBtn"),this.identityBackBtn=this.shadowRoot.querySelector("#identityBackBtn"),this.identitySupportTag=this.shadowRoot.querySelector("#identitySupportTag"),this.chatInlineIdentityBanner=this.shadowRoot.querySelector("#chatInlineIdentityBanner"),this.inlineIdentityInput=this.shadowRoot.querySelector("#inlineIdentityInput"),this.inlineIdentityBtn=this.shadowRoot.querySelector("#inlineIdentityBtn"),this.socialPickerBackBtn=this.shadowRoot.querySelector("#socialPickerBackBtn")}bindEvents(){let e=()=>this.unlockAudio();this.launcherBtn.addEventListener("click",e),window.addEventListener("click",e,{passive:!0}),window.addEventListener("keydown",e,{passive:!0}),window.addEventListener("touchstart",e,{passive:!0}),this.launcherBtn.addEventListener("click",()=>{this.toggle()}),this.shadowRoot.querySelectorAll(".welcome-close-btn, .chat-close-btn").forEach(o=>{o.addEventListener("click",()=>this.close())});let t=this.shadowRoot.querySelector(".card-active-chat");t&&t.addEventListener("click",()=>{this.goToStage("identity")}),this.identityBackBtn&&this.identityBackBtn.addEventListener("click",()=>{this.goToStage("welcome")}),this.socialPickerBackBtn&&this.socialPickerBackBtn.addEventListener("click",()=>{this.goToStage("welcome")});let i=()=>{let o=this.identityNameInput?this.identityNameInput.value.trim():"";o&&this.applyCustomerName(o,!0),this.goToStage("chat")};this.identityContinueBtn&&this.identityContinueBtn.addEventListener("click",i),this.identityNameInput&&this.identityNameInput.addEventListener("keydown",o=>{o.key==="Enter"&&(o.preventDefault(),i())});let s=this.shadowRoot.querySelector(".chat-back-btn");s&&s.addEventListener("click",()=>{this.goToStage("welcome")});let a=()=>{let o=this.inlineIdentityInput.value.trim();o&&this.applyCustomerName(o,!0)};this.inlineIdentityBtn&&this.inlineIdentityBtn.addEventListener("click",a),this.inlineIdentityInput&&this.inlineIdentityInput.addEventListener("keydown",o=>{o.key==="Enter"&&(o.preventDefault(),a())}),this.composerInput.addEventListener("input",()=>{this.composerInput.style.height="auto",this.composerInput.style.height=Math.min(this.composerInput.scrollHeight,90)+"px";let o=this.composerInput.value.trim().length>0;this.composerSendBtn.disabled=!o}),this.composerInput.addEventListener("keydown",o=>{o.key==="Enter"&&!o.shiftKey&&(o.preventDefault(),this.handleSend())}),this.composerSendBtn.addEventListener("click",()=>{this.handleSend()});let l=()=>{typeof window<"u"&&window.innerWidth<=640&&(setTimeout(()=>{this.updateViewportDimensions(),this.scrollToBottom(),this.currentStage==="chat"&&this.composerInput.scrollIntoView({block:"nearest",behavior:"smooth"})},100),setTimeout(()=>{this.updateViewportDimensions(),this.scrollToBottom()},300))};this.composerInput.addEventListener("focus",l),this.identityNameInput&&this.identityNameInput.addEventListener("focus",l)}initViewportHandler(){if(typeof window>"u")return;let e=()=>{this.isOpen&&this.updateViewportDimensions()};window.visualViewport&&(window.visualViewport.addEventListener("resize",e),window.visualViewport.addEventListener("scroll",e)),window.addEventListener("resize",e),window.addEventListener("orientationchange",()=>{setTimeout(e,200)})}updateViewportDimensions(){if(typeof window>"u")return;if(!(window.innerWidth<=640)){this.wrapperEl.style.removeProperty("--bt-viewport-height"),this.wrapperEl.style.removeProperty("--bt-viewport-top");return}if(window.visualViewport){let t=Math.round(window.visualViewport.height),i=Math.round(window.visualViewport.offsetTop);this.wrapperEl.style.setProperty("--bt-viewport-height",`${t}px`),this.wrapperEl.style.setProperty("--bt-viewport-top",`${i}px`)}else this.wrapperEl.style.setProperty("--bt-viewport-height",`${window.innerHeight}px`),this.wrapperEl.style.setProperty("--bt-viewport-top","0px")}handleSendText(e,t){let i=(e||"").trim();if(!i)return;this.composerInput.value="",this.composerInput.style.height="24px",this.composerSendBtn.disabled=!0;let s=t||i,a={id:Date.now(),conversation_id:this.sessionData?.conversation?.id||0,client_message_id:T(),sender_type:"visitor",sender_name:this.customerName||"Anda",content:s,message:i,created_at:new Date().toISOString()};this.appendMessage(a),this.emitter.emit("ui:send",a)}handleSend(){let e=this.composerInput.value.trim();e&&this.handleSendText(e)}goToStage(e){if(this.currentStage=e,e==="welcome")this.stageWelcome.style.display="flex",this.stageIdentity&&(this.stageIdentity.style.display="none"),this.stageSocialPicker&&(this.stageSocialPicker.style.display="none"),this.stageChat.style.display="none";else if(e==="identity")this.stageWelcome.style.display="none",this.stageIdentity&&(this.stageIdentity.style.display="flex"),this.stageSocialPicker&&(this.stageSocialPicker.style.display="none"),this.stageChat.style.display="none",this.identityNameInput&&(this.customerName&&(this.identityNameInput.value=this.customerName),setTimeout(()=>this.identityNameInput.focus(),150));else if(e==="social-picker")this.stageWelcome.style.display="none",this.stageIdentity&&(this.stageIdentity.style.display="none"),this.stageSocialPicker&&(this.stageSocialPicker.style.display="flex"),this.stageChat.style.display="none";else{if(this.stageWelcome.style.display="none",this.stageIdentity&&(this.stageIdentity.style.display="none"),this.stageSocialPicker&&(this.stageSocialPicker.style.display="none"),this.stageChat.style.display="flex",this.messages.length===0){let t=this.sessionData?.widget||this.sessionData?.widget_settings;if(t&&t.bot_enabled&&t.bot_welcome_message){let i=[{label:"\u{1F4E6} 1. Pembelian Produk",value:"1"},{label:"\u{1F91D} 2. Informasi & Kerjasama",value:"2"},{label:"\u{1F6E0}\uFE0F 3. Kendala Belanja Online",value:"3"},{label:"\u{1F4AC} Bicara dengan CS (YA)",value:"YA"}],s={id:0,conversation_id:this.sessionData?.conversation?.id||0,sender_type:"bot",sender_name:t.bot_name||"INDRACO Assistant",content:t.bot_welcome_message,message:t.bot_welcome_message,metadata:{is_bot:!0,is_welcome:!0,options:i},created_at:new Date().toISOString()};this.appendMessage(s)}}this.scrollToBottom(),setTimeout(()=>this.composerInput.focus(),150)}this.updateViewportDimensions()}openSocialPicker(e,t){let i=this.shadowRoot.querySelector("#socialPickerTitle"),s=this.shadowRoot.querySelector("#socialPickerHeaderTitle"),a=this.shadowRoot.querySelector("#socialPickerHeroAvatar"),l=this.shadowRoot.querySelector("#socialPickerList"),o=D(e);if(s&&(s.textContent=`Pilih Kontak ${o}`),i&&(i.textContent=`Hubungi via ${o}`),a){let c=(t[0]||{}).icon||e,b=r[c]||r[e]||r.chat;a.innerHTML=b,a.className=`social-picker-avatar social-btn-${e}`}l&&(l.innerHTML="",t.forEach(p=>{let c=document.createElement("a");c.className="social-picker-item",c.href=p.url,c.target="_blank",c.rel="noopener noreferrer";let b=p.icon||e,g=r[b]||r[e]||r.chat,m=re(p.url,e);c.innerHTML=`
          <div class="social-picker-item-avatar social-btn-${e}">
            ${g}
          </div>
          <div class="social-picker-item-info">
            <div class="social-picker-item-name">${S(p.name||o)}</div>
            ${m?`<div class="social-picker-item-sub">${S(m)}</div>`:""}
          </div>
          <div class="social-picker-item-arrow">
            ${r.chevronRight}
          </div>
        `,c.addEventListener("click",()=>{setTimeout(()=>{this.goToStage("welcome")},300)}),l.appendChild(c)})),this.goToStage("social-picker")}open(){this.isOpen=!0,this.wrapperEl.classList.add("is-open"),this.unreadCount=0,this.updateUnreadBadge(),this.updateViewportDimensions(),this.emitter.emit("widget:opened"),typeof document<"u"&&window.innerWidth<=640&&(document.documentElement.style.overflow="hidden",document.body.style.overflow="hidden"),this.currentStage==="chat"&&(this.scrollToBottom(),setTimeout(()=>this.composerInput.focus(),150))}close(){this.isOpen=!1,this.wrapperEl.classList.remove("is-open"),this.emitter.emit("widget:closed"),typeof document<"u"&&(document.documentElement.style.overflow="",document.body.style.overflow="")}toggle(){this.isOpen?this.close():this.open()}setMessages(e){this.messages=[...e],this.messagesArea.innerHTML="",e.forEach(t=>this.renderMessageBubble(t)),this.updateSnippet(),this.scrollToBottom()}unlockAudio(){try{let e=window.AudioContext||window.webkitAudioContext;if(!e)return;this.audioCtx||(this.audioCtx=new e),this.audioCtx.state==="suspended"&&this.audioCtx.resume()}catch{}}playNotificationSound(){try{if(this.unlockAudio(),!this.audioCtx)return;let e=this.audioCtx.currentTime,t=this.audioCtx.createOscillator(),i=this.audioCtx.createGain();t.type="sine",t.frequency.setValueAtTime(659.25,e),t.frequency.exponentialRampToValueAtTime(880,e+.08),i.gain.setValueAtTime(0,e),i.gain.linearRampToValueAtTime(.25,e+.02),i.gain.exponentialRampToValueAtTime(1e-4,e+.4),t.connect(i),i.connect(this.audioCtx.destination),t.start(e),t.stop(e+.4)}catch{}}appendMessage(e){this.messages.some(i=>i.id===e.id||e.client_message_id&&i.client_message_id===e.client_message_id)||(this.messages.push(e),this.renderMessageBubble(e),this.updateSnippet(),this.scrollToBottom(),e.sender_type!=="visitor"&&this.playNotificationSound(),!this.isOpen&&e.sender_type!=="visitor"&&(this.unreadCount++,this.updateUnreadBadge()))}renderMessageBubble(e){let t=e.sender_type==="visitor",i=e.sender_type==="bot",s=document.createElement("div");s.className=`msg-bubble-row ${t?"is-visitor":"is-agent"}`;let a=this.formatTime(e.created_at),l=e.content||e.message||"",o=this.formatMessageContent(l),p=t?"Anda":i?"\u{1F916} "+(e.sender_name||"BeanBot"):e.sender_name||"Agent",c=e.metadata&&Array.isArray(e.metadata.options)?e.metadata.options:null,b="";if(c&&c.length>0&&!t&&(b=`
        <div class="msg-options-container">
          ${c.map((g,m)=>`
            <button type="button" class="msg-option-btn" data-idx="${m}">
              <span>${S(g.label||g.value||"")}</span>
              <span class="msg-option-arrow">\u2192</span>
            </button>
          `).join("")}
        </div>
      `),s.innerHTML=`
      <div class="msg-sender-name" style="${i?"color: #5856D6; font-weight: 600;":""}">${S(p)}</div>
    ...
    `,c&&c.length>0&&!t){let g=s.querySelectorAll(".msg-option-btn");g.forEach((m,L)=>{m.addEventListener("click",R=>{R.preventDefault();let f=c[L];f&&(g.forEach(E=>E.disabled=!0),m.classList.add("is-selected"),this.handleSendText(f.value||f.label,f.label||f.value))})})}this.messagesArea.appendChild(s)}formatMessageContent(e){if(!e)return"";let t=S(e);return t=t.replace(/\*(.*?)\*/g,"<strong>$1</strong>"),t=t.replace(/(https?:\/\/[^\s]+)/g,'<a href="$1" target="_blank" rel="noopener noreferrer">$1</a>'),t=t.replace(/(^|[\s])(www\.[^\s]+)/g,'$1<a href="https://$2" target="_blank" rel="noopener noreferrer">$2</a>'),t}updateSnippet(){if(this.messages.length>0){let e=this.messages[this.messages.length-1],t=e.sender_type==="visitor"?"Anda: ":"",i=e.content||e.message||"";this.cardSnippetText.textContent=t+i}}updateUnreadBadge(){this.unreadCount>0?(this.unreadBadge.textContent=this.unreadCount>9?"9+":this.unreadCount.toString(),this.unreadBadge.classList.add("has-unread")):this.unreadBadge.classList.remove("has-unread")}scrollToBottom(){setTimeout(()=>{this.messagesArea.scrollTop=this.messagesArea.scrollHeight},40)}formatTime(e){try{if(!e)return"";let t=e;typeof t=="string"&&!t.includes("Z")&&!t.includes("+")&&!t.includes("T")&&(t=t.replace(" ","T")+"Z");let i=new Date(t);if(isNaN(i.getTime()))return"";let s=String(i.getHours()).padStart(2,"0"),a=String(i.getMinutes()).padStart(2,"0");return`${s}:${a}`}catch{return""}}};function S(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function D(n){return{whatsapp:"WhatsApp",instagram:"Instagram",telegram:"Telegram",messenger:"Facebook Messenger",shopee:"Shopee Store",tokopedia:"Tokopedia Store",custom:"Link Kustom",link:"Tautan Kustom"}[n.toLowerCase()]||le(n)}function re(n,e){if(!n)return"";let t=e.toLowerCase();if(t==="whatsapp"){if(n.includes("wa.me/")){let i=n.split("wa.me/")[1]?.split("?")[0]||"";return i?`+${i}`:n}}else if(t==="instagram"){if(n.includes("instagram.com/")){let i=n.split("instagram.com/")[1]?.split("/")[0]?.split("?")[0]||"";return i?`@${i}`:n}}else if(t==="telegram"&&n.includes("t.me/")){let i=n.split("t.me/")[1]?.split("/")[0]?.split("?")[0]||"";return i?`@${i}`:n}return n}function le(n){return n?n.charAt(0).toUpperCase()+n.slice(1):""}function ce(){if(typeof document>"u")return"";let n=document.currentScript;if(n&&n.src)try{return new URL(n.src,window.location.href).origin}catch{}let e=document.querySelector("script[data-project-key]");if(e&&e.src)try{return new URL(e.src,window.location.href).origin}catch{}let t=document.querySelector('script[src*="chat-widget.js"], script[src*="widget.js"]');if(t&&t.src)try{return new URL(t.src,window.location.href).origin}catch{}return""}if(typeof window<"u")try{console.log("%c[BeanTalk]%c Universal Chat Widget Started","background: #0071E3; color: #ffffff; padding: 2px 6px; border-radius: 4px; font-weight: bold;","color: inherit; font-weight: 500;")}catch{}var k=class{constructor(e){this.sessionData=null;this.initialized=!1;this.sendQueue=[];this.isSending=!1;this.options=e,this.emitter=new C,console.log("[BeanTalk] Initializing widget instance for project:",e.projectKey);let t=ce(),i=e.apiUrl||t||(typeof window<"u"?window.location.origin:"");this.api=new I(e.projectKey,i),this.ui=new B(e,this.emitter),this.transport=new M(this.api,this.emitter),this.bindEvents(),this.bootstrap()}isReady(){return this.initialized}async processSendQueue(){if(!(this.isSending||this.sendQueue.length===0)){for(this.isSending=!0;this.sendQueue.length>0;){let e=this.sendQueue.shift(),t=this.sessionData?.conversation?.id||0,i=this.options.visitorUuid||A(),s=w();try{let a=await this.api.sendMessage(t,{visitor_uuid:i,client_message_id:e.client_message_id||T(),message:e.content||e.message||"",sender_name:e.sender_name||s||"Tamu",page_url:window.location.href,page_title:document.title});if(a.success&&a.data){let l=a.data.conversation_id;l&&(!this.sessionData?.conversation?.id||this.sessionData.conversation.id!==l)&&(this.sessionData||(this.sessionData={}),this.sessionData.conversation?this.sessionData.conversation.id=l:this.sessionData.conversation={id:l,status:"open"},O(l),this.transport.start(l,a.data.id||0)),this.emitter.emit("message:sent",a.data)}}catch(a){console.error("[BeanTalk] Gagal mengirim pesan:",a)}}this.isSending=!1,this.transport.pollNow()}}bindEvents(){this.emitter.on("ui:send",e=>{this.sendQueue.push(e),this.processSendQueue()}),this.emitter.on("customer:rename",async e=>{let t=this.options.visitorUuid||A();try{await this.api.updateProfile(t,e)}catch(i){console.warn("[BeanTalk] Gagal update nama profil pengunjung:",i)}}),this.emitter.on("message:received",e=>{this.ui.appendMessage(e),this.emitter.emit("message",e)}),this.emitter.on("widget:opened",()=>{this.transport.setWidgetOpen(!0),this.transport.pollNow()}),this.emitter.on("widget:closed",()=>{this.transport.setWidgetOpen(!1)})}async bootstrap(){let e=this.options.visitorUuid||A(),t=w();try{let i=await this.api.initSession(e,void 0,t||void 0);if(i.success&&i.data){this.sessionData=i.data,this.ui.setSessionData(i.data);let s=i.data.conversation;if(s&&s.id){O(s.id);let a=0;s.messages&&s.messages.length>0&&(a=Math.max(...s.messages.map(l=>l.id))),this.transport.start(s.id,a)}this.initialized=!0,console.log("[BeanTalk] Session established successfully:",{brand:i.data.project?.name,customerCode:i.data.visitor?.customer_code,conversationId:i.data.conversation?.id||"New Thread"}),this.emitter.emit("ready",i.data)}else console.warn("[BeanTalk] Init session warning:",i.error?.message)}catch(i){console.error("[BeanTalk] Failed to initialize chat session:",i)}}open(){console.log("[BeanTalk] Opening chat widget window"),this.ui.open()}close(){console.log("[BeanTalk] Closing chat widget window"),this.ui.close()}toggle(){this.ui.toggle()}on(e,t){return this.emitter.on(e,t),this}sendMessage(e){if(!e.trim())return;let t={id:Date.now(),conversation_id:this.sessionData?.conversation?.id||0,client_message_id:T(),sender_type:"visitor",sender_name:"Anda",content:e.trim(),message:e.trim(),created_at:new Date().toISOString()};this.ui.appendMessage(t),this.emitter.emit("ui:send",t)}},de=k,x=null,pe=n=>u.init(n),he=()=>u.open(),ue=()=>u.close(),ge=()=>u.toggle(),me=(n,e)=>u.on(n,e),xe=n=>u.sendMessage(n),be=()=>u.getInstance(),u={init(n){return x||(console.log("[BeanTalk] Creating singleton widget instance"),x=new k(n)),x},open(){x?x.open():console.warn("[BeanTalk] Widget instance not yet initialized")},close(){x?.close()},toggle(){x?.toggle()},on(n,e){x?.on(n,e)},sendMessage(n){x?.sendMessage(n)},getInstance(){return x}};if(typeof window<"u"){let n=function(){let e=document.querySelectorAll("script[data-project-key]");if(e.length>0){let t=e[0],i=t.getAttribute("data-project-key"),s="";if(t.src)try{s=new URL(t.src,window.location.href).origin}catch{}let a=t.getAttribute("data-api-url")||s||void 0,l=t.getAttribute("data-color")||void 0,o=t.getAttribute("data-brand-name")||t.getAttribute("data-store-name")||void 0,p=t.getAttribute("data-support-title")||void 0;console.log("[BeanTalk] Found embed tag on page:",{projectKey:i,apiUrl:a,brandName:o}),i&&!x&&u.init({projectKey:i,apiUrl:a,accentColor:l,brandName:o,storeName:o,supportTitle:p})}};fe=n,window.ChatWidget=u,window.UniversalChatMe=k,setTimeout(()=>{try{window.BeanTalk&&Object.assign(window.BeanTalk,u)}catch{}},0),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",n):n()}var fe,ve=u;return ne(ye);})();
