...
- **Filter Tabs**:
  - `All`: Seluruh percakapan di project.
  - `Open`: Percakapan yang belum terselesaikan.
  - `Pending`: Menunggu tanggapan customer.
  - `Closed`: Percakapan yang sudah diarsipkan.
  - `Mine`: Percakapan yang ditugaskan khusus ke agen yang sedang login.
- **Kartu Percakapan**:
  - Nama customer & avatar/inisial
  - Snippet pesan terakhir
  - Timestamp relatif (misal: "2m", "1h", "yesterday")
  - Badge angka unread berwarna kontras
  - Tag sumber (misal: badge kecil "Shopify", "Website", "Laravel")
